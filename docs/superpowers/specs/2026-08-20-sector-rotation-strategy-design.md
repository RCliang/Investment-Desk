...
## v2 实验（2026-08-21，方向 1+2 已实现并 A/B 验证）

退出规则参数化：`stop_mode`（fixed=入场×0.9 / atr=入场−mult×ATR14）、`breakdown_buffer`（close 需低于 MA20×(1−buffer) 才算破位）。API `POST /rotation/backtest` 已支持；v1 行为可精确复现（基准行与上文完全一致）。

| 配置 | 总收益 | 夏普 | 最大回撤 | 胜率 | 均持仓 |
|---|---|---|---|---|---|
| v1 基准（固定 10%，骨架） | +105.7% | 0.668 | 36.7% | — | 11.5d |
| v2 ATR2.0 + 缓冲3%（骨架） | **+189.0%** | **0.867** | **28.8%** | 43.4% | 13.6d |
| v1 基准（固定 10%，含主力因子） | +64.6% | 0.517 | 36.7% | 38.7% | 11.4d |
| v2 ATR2.0 + 缓冲3%（含主力因子） | +123.7% | 0.714 | 28.2% | 43.9% | 13.5d |
| ATR3.0 + 缓冲3%（含主力因子，对照） | +107.4% | 0.650 | 34.0% | 44.1% | 14.2d |
| 池内等权基准 | +294.5% | 0.928 | 44.4% | — | — |

结论：两个反鞭打旋钮独立有效、组合更优（含主力因子组 +59pp，纯骨架组 +83pp），夏普 0.517→0.714 / 0.668→0.867，回撤 36.7%→28.2-28.8%（基准 44.4%），胜率与持仓天数同向改善；止损退出从 59 笔降至 38 笔（ATR2.0）、破位退出从 104 笔降至 73 笔（缓冲 3%），强制退出被计划内调仓替代（21→44 笔）。ATR 2.0 优于 3.0（3.0 过宽，止损缺失转为破位退出，整体更差）。与基准的差距从 189pp 收窄到 105pp；风险调整后夏普 0.867 已接近基准 0.928，且回撤优势显著（28.8% vs 44.4%）——中期趋势策略的价值主张落在"以小得多的回撤参与趋势"上。

主力因子在两种退出规则下均拖累收益（−65pp），与近端负 IC 一致，ICIR 已自动降权；待 200 日窗口累积满一年后重估。默认参数仍为 v1（fixed/buffer=0）以保回测可比性；推荐实盘参考配置 `stop_mode=atr&atr_mult=2.0&breakdown_buffer=0.03&use_fund_flow_factors=false`。
