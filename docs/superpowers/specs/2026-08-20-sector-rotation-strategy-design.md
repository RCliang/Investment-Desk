...
## v3 实验（2026-08-22，方向 3/4/5：保留持仓 / 冷却回补 / 因子反向）

新参数：`keep_in_trend`（调仓保留仍在多头排列的旧持仓，槽位受目标组合数量约束）、`reentry_enabled` + `reentry_cooldown`（退出后现金在上次目标组合内趋势重立时回补）、`fund_flow_direction`（主力因子反向开关）。全部在 v2 最优退出规则（ATR2.0+缓冲3%）之上叠加，基准行 bit 级复现。

| 配置 | 总收益 | 夏普 | 最大回撤 | 笔数 | 胜率 | 均持仓 | 月换手 |
|---|---|---|---|---|---|---|---|
| E0 v2最优（骨架） | +189.0% | 0.867 | 28.8% | 235 | 43.4% | 13.6d | 77.5% |
| E1 +保留趋势持仓 | **+225.5%** | 0.860 | 34.4% | 200 | 37.5% | 15.5d | 66.1% |
| E2 +回补（冷却5日） | +161.9% | 0.776 | 34.1% | 291 | 43.0% | 12.1d | 74.1% |
| E3 +保留+回补 | +115.2% | 0.594 | 56.8% | 270 | 33.3% | 14.8d | 48.9% |
| E4 E3+主力因子反向 | +90.1% | 0.531 | 56.8% | 262 | 35.1% | — | 47.9% |
| E5 E3+主力因子正向 | +99.5% | 0.555 | 56.8% | 268 | 34.7% | — | 48.7% |

结论（两个正贡献为零、三个负贡献，如实记录）：

1. **保留趋势持仓（E1）是收益换风险的旋钮而非免费午餐**：+36.5pp 收益、换手 77.5%→66.1%，但回撤 28.8%→34.4%、胜率下降，夏普持平（0.860 vs 0.867）。机制：板块层轮动减仓信号被个股层"仍在趋势"否决，牛市多吃趋势、板块切换时多挨刀。追求收益可开，防守型定位则不开。
2. **趋势重立回补（E2）被证伪**：−27pp。多头排列重立必然发生在反弹之后，回补买点系统性偏晚，随即再被止损打出（291 笔）。原诊断中"空仓损耗"的实际修复手段是减少被迫退出（v2 已做），而不是退出后追回。
3. **保留+回补组合（E3）严重劣化**（回撤 56.8%）：保留占满槽位后回补现金集中涌入少数票，集中度爆炸。两者互斥，不可同开。
4. **主力因子反向（E4）劣于正向（E5）**：负 IC 的正确应对是 ICIR 降权（已自动生效）而非机械反向——反向因子在动量正常期同样失效。
5. **推荐配置维持 v2（E0）**：`stop_mode=atr&atr_mult=2.0&breakdown_buffer=0.03&use_fund_flow_factors=false`，进攻型变体加 `keep_in_trend=true`（收益 +36pp、回撤 +6pp 的显式取舍）。
