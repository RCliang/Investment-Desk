...
- **Tab icon:** Inline SVG components in `frontend/src/auth/icons.tsx` (`LockIcon` / `UnlockIcon` / `LogoutIcon`), stroke = `currentColor`, no `@ant-design/icons` dependency (removed in 2026-06-26 slim-down).
...
| `frontend/src/auth/icons.tsx` | Inline SVG icon components (`LockIcon` / `UnlockIcon` / `LogoutIcon`); stroke uses `currentColor` so CSS controls color |
...
| `frontend/src/App.tsx` | Wrap with `AdminAuthProvider`; rename tab label; conditional `LockIcon` / `UnlockIcon` (inline SVG from `./auth/icons`); click intercept → open modal |
...
## Task 5: Frontend — `icons.tsx` + `admin.css` + `AdminLoginModal`
...
- Produces: `<AdminLoginModal open={bool} onClose={fn} errorMsg={string} />` component; `LockIcon` / `UnlockIcon` / `LogoutIcon` SVG components in `./auth/icons`.

- [ ] **Step 1: Create `frontend/src/auth/icons.tsx`**

Tiny module — three SVG components (`LockIcon` / `UnlockIcon` / `LogoutIcon`). Inline SVG, **not** `@ant-design/icons` (that package was removed in the 2026-06-26 slim-down). Stroke = `currentColor` so the icons inherit CSS color (`var(--ink)` on paper background).
...
- [ ] **Step 2: Create `frontend/src/auth/admin.css`**
...
- [ ] **Step 3: Create `frontend/src/auth/AdminLoginModal.tsx`**
...
```bash
cd E:/2026projects/Investment-Desk
git add frontend/src/auth/icons.tsx frontend/src/auth/admin.css frontend/src/auth/AdminLoginModal.tsx
git commit -m "$(cat <<'EOF'
feat(frontend): add inline SVG icons + paper-style AdminLoginModal
...
The integration task. Wraps app in `AdminAuthProvider`, renames tab label, adds LockIcon/UnlockIcon (inline SVG), intercepts click to open modal if not admin.
...
公司深度分析 with conditional LockIcon (when locked) / UnlockIcon
(when authenticated) inline SVG icons. Clicking the locked tab opens AdminLoginModal
